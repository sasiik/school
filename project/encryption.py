...
class Encryptor:
    ...
    def encrypt_text(self, text, enc_key):
        ...
        for i, text_sym in enumerate(text):
            encrypted_sym = ord(
                text_sym) + ord(enc_key[i] if i < len(enc_key) else enc_key[i % len(enc_key)]) + self.nonce
    ...
    def decrypt_text(self, enc_text, enc_key):
        ...
        for i, enc_text_sym in enumerate(enc_text):
            decrypted_sym = ord(
                enc_text_sym) - ord(enc_key[i] if i < len(enc_key) else enc_key[i % len(enc_key)]) - self.nonce
